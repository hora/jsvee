(function($) {
  ...
  JSVEE.handlers.global = function(ready, area, element, params) {
    ...
    if (element.hasClass('jsvee-operator')) {
      ...
      if (element.attr('data-type') == 'p' && element.attr('data-name') == '.') {
        var ref = area.find('.jsvee-instance[data-id="' + element.prev().data('id') + '"]');
        var prop = element.find('.jsvee-property').data('name');
        var result = ref.find('.jsvee-variable[data-name="' + prop + '"] .jsvee-value').text();
        element.prev().remove();
        ready(JSVEE.utils.ui.findOrCreateValue(area, result, 'int'));
        return;
      }
    ...
    } else if (element.hasClass('jsvee-function') && (element.attr('data-name') === 'console.log')) {
